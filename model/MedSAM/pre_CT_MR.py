...
parser.add_argument("--split_ratio",type=float,default=0.8,help="Split ratio for training and testing data, [default: 0.8]")
...
def preprocess(name, npz_path):
    ...
    if len(z_index) > 0:
        ...
        np.savez_compressed(join(npz_path, prefix + gt_name.split(gt_name_suffix)[0]+'.npz'), imgs=img_roi, gts=gt_roi, z_index = z_index,spacing=img_sitk.GetSpacing())
...
if __name__ == "__main__":
    split_num = int(len(names) * split_ratio)
    tr_names = names[:split_num]
    ts_names = names[split_num:]
    print(len(tr_names), len(ts_names))
