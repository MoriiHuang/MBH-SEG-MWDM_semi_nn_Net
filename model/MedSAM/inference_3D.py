...
def MedSAM_infer_npz(gt_path_file):
    ...
    if save_mode == 'npz':
        ...
        makedirs(join(pred_save_dir), exist_ok=True)
    if (not isfile(join(pred_save_dir, task_folder, npz_name))) or overwrite:
        ...
        if save_mode == 'nii':
            nii_name = npz_name.replace('CT_Abd_','').replace('.npz', '.nii.gz')
            if ref_nii_file_path is not None:
                ref_path = join(ref_nii_file_path, nii_name)
                ref_nii = nib.load(ref_path)
                ref_array = ref_nii.get_fdata()
                seg = np.transpose(seg_3D, (1, 2, 0))
                ## channel 2 rotate 270 degree
                seg = np.rot90(seg, 3, (0, 1))
                ## channel 2 flip up-down
                seg = np.flip(seg, axis=1)
                save_seg = np.zeros_like(ref_array)
                save_seg[:,:,z_index] = seg
                affine = ref_nii.affine
                nii_image = nib.Nifti1Image(save_seg, affine)
                nib.save(nii_image, join(pred_save_dir, nii_name))
        elif save_mode == 'npz':
            np.savez_compressed(
