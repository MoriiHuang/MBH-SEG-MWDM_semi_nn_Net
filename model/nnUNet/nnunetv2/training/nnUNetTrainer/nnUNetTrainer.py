...
from nnunetv2.utilities.AttentionModel import ResUNet
...
from nnunetv2.training.loss.dice import get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss,SoftDiceLossNew
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'),unsupervised:bool = True, attention:bool=False,loss_method:int = 1):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           ...
                                                           enable_deep_supervision=True,attention=self.attention).to(self.device)
            # compile network for free speedup
            if self._do_i_compile():
                self.print_to_log_file('Compiling network...')
                self.network = torch.compile(self.network)
            # print("calculating para")
            # summary(self.network, input_size=(49, 1, 256, 256))
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        if attention:
            return get_network_from_plans(plans_manager, dataset_json, configuration_manager,
                                      num_input_channels, deep_supervision=enable_deep_supervision,mapping={
                               'PlainConvUNet': ResUNet,
    ...
    def _build_loss(self):
        ...
        elif self.loss_method ==2:
            if self.label_manager.has_regions:
                loss = DC_and_BCE_loss({},
                                    {'batch_dice': self.configuration_manager.batch_dice,
                                        'do_bg': True, 'smooth': 1e-5, 'ddp': self.is_ddp},
                                    use_ignore_label=self.label_manager.ignore_label is not None,
                                    dice_class=SoftDiceLossNew)
            else:
                loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                    'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
                                    ignore_label=self.label_manager.ignore_label, dice_class=SoftDiceLossNew)
