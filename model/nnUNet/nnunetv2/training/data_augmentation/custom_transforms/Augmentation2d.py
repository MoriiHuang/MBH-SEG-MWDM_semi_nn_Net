...
class batch_tensor_to_img_aug_and_revort_to_batch_tensor:
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
        self.mean = mean
        self.std = std
        self.normalize = transforms.Normalize(mean, std)
        ...
        self.strong_aug = strong_img_aug(2)
        
    def __call__(self, in_tensor):
        in_tensor = torch.repeat_interleave(in_tensor, 3, dim=1)
        # print("="*100, in_tensor.size())
        results = []
        batch_size = in_tensor.size(0)
        for i in range(batch_size):
            res = self.to_pil(in_tensor[i])
            res = self.strong_aug(res)
            res = self.to_tensor(res)
...
if __name__ == "__main__":
    tensor = torch.randn(3,1,512,448)
    print(tensor.shape)
    btarbt=batch_tensor_to_img_aug_and_revort_to_batch_tensor()
    tensor = btarbt(tensor)
    print(tensor.shape)
