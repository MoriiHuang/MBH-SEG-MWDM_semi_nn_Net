...
def cal_confidence(pred_u,num_classes):
    ...
    # confidence = confidence.mean(dim=[])  # 1*C
...
if __name__ == "__main__":
    ...
    ### test sample
    unlabeled_image = torch.rand((14, 1, 512, 448))
    labeled_image = torch.rand((14, 1, 512, 448))
    labeled_mask = torch.randint(0, 5, (14, 512, 448))  
    

    input_shape = (14, 512, 448)
    num_classes = 5
    ...
    pred_probs = torch.permute(pred_probs, (0, 3, 1, 2))
    pred_u = F.softmax(pred_probs, dim=1)
    logits_u, label_u = torch.max(pred_u, dim=1)
    conf = cal_confidence(pred_u,num_classes=5)
    unlabeled_image, unlabeled_mask, unlabeled_logits= cut_mix_label_adaptive(unlabeled_image,label_u,logits_u,labeled_image,labeled_mask,conf)
    print(unlabeled_image.shape,unlabeled_mask.shape,unlabeled_logits.shape)  # 应该是 torch.Size([14, 1, 512, 448])