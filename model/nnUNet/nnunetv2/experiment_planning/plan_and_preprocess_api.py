...
from nnunetv2.utilities.utils import get_filenames_of_train_images_and_targets,get_filenames_of_train_images_and_targets_unsupervised
...
def preprocess_dataset(dataset_id: int,
    ...
    for n, c in zip(num_processes, configurations):
        ...
        preprocessor = configuration_manager.preprocessor_class(verbose=verbose,supvised=unsupervised)
    ...
    if unsupervised:
        dataset,_ = get_filenames_of_train_images_and_targets_unsupervised(join(nnUNet_raw, dataset_name), dataset_json)
    else:
...
def preprocess(dataset_ids: List[int],
    ...
    for d in dataset_ids:
        preprocess_dataset(d, plans_identifier, configurations, num_processes, verbose,unsupervised)
