...
def export_prob_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    np.savez_compressed(output_file_truncated + '_0000.npz', probabilities=probabilities_final,spacing=properties_dict['sitk_stuff']['spacing'],direction=properties_dict['sitk_stuff']['direction'],origin=properties_dict['sitk_stuff']['origin'],props=None)
    print(f"Saved probabilities to {output_file_truncated}.npz")
    del probabilities_final, ret

    rw = plans_manager.image_reader_writer_class()
    output_file_truncated_seg = output_file_truncated.split('/')[:-2]
    output_file_truncated_seg_folder = '/'.join(output_file_truncated_seg) + '/seg'
    if not os.path.exists(output_file_truncated_seg_folder):
        os.makedirs(output_file_truncated_seg_folder)
    output_file_truncated_seg = '/'.join(output_file_truncated_seg) + '/seg/'+output_file_truncated.split('/')[-1]
    print(f'output_file_truncated_seg: {output_file_truncated_seg}')
    rw.write_seg(segmentation_final, output_file_truncated_seg + dataset_json_dict_or_file['file_ending'],
                 properties_dict)
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            if i == 0:
                ...
                if trainer_name == 'nnUNetTrainerAugSeg':
    ...
    def predict_from_files_with_prob_only(self,
        ...
        if output_folder is not None:
            my_init_kwargs = {}
            for k in inspect.signature(self.predict_from_files_with_prob_only).parameters.keys():
        ...
        return self.predict_from_data_iterator_prob_only(data_iterator, save_probabilities, num_processes_segmentation_export)
    
    def predict_from_data_iterator_prob_only(self,
                                   data_iterator,
                                   save_probabilities: bool = True,
                                   num_processes_segmentation_export: int = default_num_processes):
        """
    ...
    def predict_from_files(self,
        ...
        return self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export,use_cam=cam,cam_type=cam_type)
    ...
    def predict_from_data_iterator(self,
                                   data_iterator,
                                   save_probabilities: bool = False,
                                   num_processes_segmentation_export: int = default_num_processes,use_cam:bool = False,cam_type:str = "2d"):
        """
        each element returned by data_iterator must be a dict with 'data', 'ofile' and 'data_properties' keys!
        If 'ofile' is None, the result will be returned instead of written to a file
        """
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if use_cam:
                    print("start processing cam")
                
                    sem_class_to_idx = {cls: idx for (idx, cls) in enumerate(sem_classes)}

                    target_layers = [self.network.decoder.seg_layers[5]] 
                    FGR_category = sem_class_to_idx["FGR"]
                    class SemanticSegmentationTarget:
                        def __init__(self, category, mask,scale_factor=1):
                    ...
                    cam_data, slicer_revert_padding = pad_nd_image(data, self.configuration_manager.patch_size,
                                                            'constant', {'value': 0}, True,
                                                      None)
                    if cam_type == '2d':
                        slicers = self._internal_get_sliding_window_slicers(cam_data.shape[1:])
                    else:
                        slicers = self._internal_get_sliding_window_slicers(data.shape[1:])
                   
                    predicted_cam = torch.zeros((1, *cam_data.shape[1:]),
                                                        dtype=torch.half,
                                                        device=self.device)
                    ...
                    for params in self.list_of_parameters:
                        ...
                        predicted_cam_file = torch.zeros((1, *cam_data.shape[1:]),
                                                        dtype=torch.half,
                                                        device=self.device)
                        n_predictions_cam_file = torch.zeros(cam_data.shape[1:], dtype=torch.half,
                                                        device=self.device)
                    
                        with GradCAM(self.network, target_layers=target_layers,
                            ...
                            for sl in slicers:
                                ...
                                if cam_type == '2d':
                                    with torch.no_grad():
                                        ...
                                        sd_prediction = self._internal_maybe_mirror_and_predict(workon)[0]
                                        sd_predicted_probabilities = self.label_manager.apply_inference_nonlin(sd_prediction)
                                        car_mask = self.label_manager.convert_probabilities_to_segmentation(sd_predicted_probabilities).unsqueeze(0).detach().cpu().numpy()

                                else:
                                    with torch.no_grad():
                                        td_prediction = self._internal_maybe_mirror_and_predict(workon)[0]
                                        td_predicted_probabilities = self.label_manager.apply_inference_nonlin(td_prediction)
                                        car_mask = self.label_manager.convert_probabilities_to_segmentation(td_predicted_probabilities).unsqueeze(0).detach().cpu().numpy()
                                car_mask_float = np.float32(car_mask == FGR_category)
                                targets = [SemanticSegmentationTarget(FGR_category, car_mask_float)]
                                grayscale_cam = torch.from_numpy(cam(input_tensor=workon,targets=targets)[0, :]).to(self.device)
                    ...
                    predicted_cam /= len(self.list_of_parameters)
                    slices_cam = torch.split(predicted_cam, 1, dim=1)
                    slices_ori = torch.split(data,1,dim=1)
                    index = 0
                    # 转换并保存每个小张量
                    last_infer_index = ofile.rfind("infer")
                    if last_infer_index != -1:
                        # 进行替换
                        tmp_path = ofile[:last_infer_index] + "cam" + ofile[last_infer_index + len("infer"):]
                        tmp_path_ori = ofile[:last_infer_index] + "ori" + ofile[last_infer_index + len("infer"):]
                    new_path = Path(*tmp_path.rsplit("/")[:-1])
                    file_name = tmp_path.rsplit("/")[-1]
                    newPath = Path(join(new_path,file_name))
                    newPath.mkdir(parents=True,exist_ok=True)
                    for slice_cam,slice_ori in zip(slices_cam,slices_ori):
                        img_array_gray = slice_ori.squeeze().numpy()
                        img_array_gray_normalized = (img_array_gray - img_array_gray.min()) / (img_array_gray.max() - img_array_gray.min())
                        img_array_rgb = cv2.cvtColor(img_array_gray_normalized, cv2.COLOR_GRAY2RGB)
                        ...
                        image.save(f"{newPath}/{file_name}_{index}.png")
                        index+=1
                if ofile is not None:
                    # this needs to go into background processes
                    # export_prediction_from_logits(prediction, properties, configuration_manager, plans_manager,
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-cam', type=bool, default=False, required=False)
    print(
...
def predict_entry_point():
    ...
    predictor.predict_from_files(args.i, args.o, save_probabilities=args.save_probabilities,
                                 ...
                                 part_id=args.part_id,cam = args.cam,cam_type=args.c)
